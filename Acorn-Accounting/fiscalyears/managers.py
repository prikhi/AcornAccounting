...
class FiscalYearManager(CachingManager):
    """A Custom Manager for the :class:`~.models.FiscalYear` model."""
    def current_start(self):
        ...
        first day and month after the Second Latest
        :class:`~.models.FiscalYear` will be returned.

        :returns: The starting date of the current
                :class:`~.models.FiscalYear`.
        :rtype: :class:`datetime.date` or :obj:`None`

        """
        if self.exists():
            if self.count() > 1:
                second_latest = self.order_by('-date')[1]
                return second_latest.date + relativedelta(months=1)
            else:
                current_year = self.get()
